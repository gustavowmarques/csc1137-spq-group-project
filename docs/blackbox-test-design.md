...
This black box testing has been done by the team manually. The goal for this phase was to set up the skeletal of the project and lay out a detailed testing plan. We have already identified the tools and frameworks we will be using in the upcoming phases as the codebase gets bigger and the tools have been explained in the SQA plan.
...
### 2.1 Medicine Name for Prescription
...
- EP-ACNT-1: returning user with a correct role, then respective dashboard shown.
...
The known conflict pairs in our system (for now) are:
- warfarin conflicts with aspirin.
- warfarin conflicts with ibuprofen.
